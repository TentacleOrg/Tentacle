import 'package:test/test.dart';
import 'package:tentacle/tentacle.dart';

// tests for DiscoverMoviesStudioStudioIdGet200Response
void main() {
  final instance = DiscoverMoviesStudioStudioIdGet200ResponseBuilder();
  // TODO add properties to the builder and call build()

  group(DiscoverMoviesStudioStudioIdGet200Response, () {
    // num page
    test('to test the property `page`', () async {
      // TODO
    });

    // num totalPages
    test('to test the property `totalPages`', () async {
      // TODO
    });

    // num totalResults
    test('to test the property `totalResults`', () async {
      // TODO
    });

    // ProductionCompany studio
    test('to test the property `studio`', () async {
      // TODO
    });

    // BuiltList<MovieResult> results
    test('to test the property `results`', () async {
      // TODO
    });
  });
}
