import 'package:test/test.dart';
import 'package:tentacle/tentacle.dart';

// tests for DiscoverMoviesGenreGenreIdGet200Response
void main() {
  final instance = DiscoverMoviesGenreGenreIdGet200ResponseBuilder();
  // TODO add properties to the builder and call build()

  group(DiscoverMoviesGenreGenreIdGet200Response, () {
    // num page
    test('to test the property `page`', () async {
      // TODO
    });

    // num totalPages
    test('to test the property `totalPages`', () async {
      // TODO
    });

    // num totalResults
    test('to test the property `totalResults`', () async {
      // TODO
    });

    // Genre genre
    test('to test the property `genre`', () async {
      // TODO
    });

    // BuiltList<MovieResult> results
    test('to test the property `results`', () async {
      // TODO
    });
  });
}
