import 'package:test/test.dart';
import 'package:tentacle/tentacle.dart';

// tests for PluginInstallationCancelledMessageData
void main() {
  final instance = PluginInstallationCancelledMessageDataBuilder();
  // TODO add properties to the builder and call build()

  group(PluginInstallationCancelledMessageData, () {
    // Gets or sets the Id.
    // String guid
    test('to test the property `guid`', () async {
      // TODO
    });

    // Gets or sets the name.
    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // Gets or sets the version.
    // String version
    test('to test the property `version`', () async {
      // TODO
    });

    // Gets or sets the changelog for this version.
    // String changelog
    test('to test the property `changelog`', () async {
      // TODO
    });

    // Gets or sets the source URL.
    // String sourceUrl
    test('to test the property `sourceUrl`', () async {
      // TODO
    });

    // Gets or sets a checksum for the binary.
    // String checksum
    test('to test the property `checksum`', () async {
      // TODO
    });

    // InstallationInfoPackageInfo packageInfo
    test('to test the property `packageInfo`', () async {
      // TODO
    });

  });
}
