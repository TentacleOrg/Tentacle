import 'package:test/test.dart';
import 'package:tentacle/tentacle.dart';

// tests for PlexDevice
void main() {
  final instance = PlexDeviceBuilder();
  // TODO add properties to the builder and call build()

  group(PlexDevice, () {
    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // String product
    test('to test the property `product`', () async {
      // TODO
    });

    // String productVersion
    test('to test the property `productVersion`', () async {
      // TODO
    });

    // String platform
    test('to test the property `platform`', () async {
      // TODO
    });

    // String platformVersion
    test('to test the property `platformVersion`', () async {
      // TODO
    });

    // String device
    test('to test the property `device`', () async {
      // TODO
    });

    // String clientIdentifier
    test('to test the property `clientIdentifier`', () async {
      // TODO
    });

    // String createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // String lastSeenAt
    test('to test the property `lastSeenAt`', () async {
      // TODO
    });

    // BuiltList<String> provides
    test('to test the property `provides`', () async {
      // TODO
    });

    // bool owned
    test('to test the property `owned`', () async {
      // TODO
    });

    // String ownerID
    test('to test the property `ownerID`', () async {
      // TODO
    });

    // bool home
    test('to test the property `home`', () async {
      // TODO
    });

    // String sourceTitle
    test('to test the property `sourceTitle`', () async {
      // TODO
    });

    // String accessToken
    test('to test the property `accessToken`', () async {
      // TODO
    });

    // String publicAddress
    test('to test the property `publicAddress`', () async {
      // TODO
    });

    // bool httpsRequired
    test('to test the property `httpsRequired`', () async {
      // TODO
    });

    // bool synced
    test('to test the property `synced`', () async {
      // TODO
    });

    // bool relay
    test('to test the property `relay`', () async {
      // TODO
    });

    // bool dnsRebindingProtection
    test('to test the property `dnsRebindingProtection`', () async {
      // TODO
    });

    // bool natLoopbackSupported
    test('to test the property `natLoopbackSupported`', () async {
      // TODO
    });

    // bool publicAddressMatches
    test('to test the property `publicAddressMatches`', () async {
      // TODO
    });

    // bool presence
    test('to test the property `presence`', () async {
      // TODO
    });

    // BuiltList<PlexConnection> connection
    test('to test the property `connection`', () async {
      // TODO
    });

  });
}
